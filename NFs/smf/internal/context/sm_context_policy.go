...
func (c *SMContext) MarkQosFlowToBeRemove(qosID string, ruleID uint8) {
	qfi, ok := c.qosDataToQFI[qosID]
	if !ok {
		return
	}
	if qosFlow := c.AdditonalQosFlows[qfi]; qosFlow != nil {
		qosFlow.State = QoSFlowToBeRemove
	}
	if ruleID != 0 {
		c.PendingQosFlowReleases[qfi] = ruleID
	}
	c.QFIGenerator.FreeID(int64(qfi))
	delete(c.qosDataToQFI, qosID)
}
...
func (c *SMContext) ApplyPccRules(decision *models.SmPolicyDecision) error {
	...
	for id, pccModel := range decision.PccRules {
		if pccModel == nil {
			...
			if srcPcc := c.PCCRules[id]; srcPcc != nil {
				c.PreRemoveDataPath(srcPcc.Datapath)
				ruleID := c.PCCRuleIDToQoSRuleID[id]
				for _, qosID := range srcPcc.RefQosData {
					c.MarkQosFlowToBeRemove(qosID, ruleID)
				}
				delete(c.PCCRuleIDToQoSRuleID, id)
				srcTcData := c.TrafficControlDatas[srcPcc.RefTcDataID()]
