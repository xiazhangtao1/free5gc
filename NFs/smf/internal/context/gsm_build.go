...
func BuildGSMPDUSessionModificationCommand(smContext *SMContext) ([]byte, error) {
	...
	qoSRules := nasType.QoSRules{}
	for qfi, ruleID := range smContext.PendingQosFlowReleases {
		if ruleID == 0 {
			logger.GsmLog.Warnf("Skip deleting QoS rule for QFI[%d]: rule ID is missing\n", qfi)
			continue
		}
		qoSRules = append(qoSRules, nasType.QoSRule{
			Identifier: ruleID,
			Operation:  nasType.OperationCodeDeleteExistingQoSRule,
		})
	}
	for _, pccRule := range smContext.PCCRules {
		if pccRule.QFI == 0 {
		...
		qosFlow := smContext.AdditonalQosFlows[pccRule.QFI]
		if qosFlow == nil || (qosFlow.State != QoSFlowUnset && qosFlow.State != QoSFlowToBeModify) {
			continue
		}

		var operation nasType.QoSRuleOperationCode
		if qosFlow.State == QoSFlowToBeModify {
			operation = nasType.OperationCodeModifyExistingQoSRuleAndReplaceAllPacketFilters
		} else {
			operation = nasType.OperationCodeCreateNewQoSRule
		}

		qosRule, err := pccRule.BuildNasQoSRule(smContext, operation)
		if err != nil {
			logger.GsmLog.Warnln("Create QoS rule from pcc error ", err)
			continue
		}

		ruleID := smContext.PCCRuleIDToQoSRuleID[pccRule.PccRuleId]
		if ruleID == 0 {
			allocatedRuleID, err := smContext.QoSRuleIDGenerator.Allocate()
			if err != nil {
				return nil, err
			}
			ruleID = uint8(allocatedRuleID)
			smContext.PCCRuleIDToQoSRuleID[pccRule.PccRuleId] = ruleID
		}
		qosRule.Identifier = ruleID
		qoSRules = append(qoSRules, *qosRule)
	}
	...
	authDescs := nasType.QoSFlowDescs{}
	for qfi := range smContext.PendingQosFlowReleases {
		authDescs = append(authDescs, nasType.QoSFlowDesc{
			QFI:           qfi,
			OperationCode: nasType.OperationCodeDeleteExistingQoSFlowDescription,
		})
	}
	for _, qosFlow := range smContext.AdditonalQosFlows {
		if qosFlow.State != QoSFlowUnset && qosFlow.State != QoSFlowToBeModify {
			continue
		}

		operation := nasType.OperationCodeCreateNewQoSFlowDescription
		if qosFlow.State == QoSFlowToBeModify {
			operation = nasType.OperationCodeModifyExistingQoSFlowDescription
		}

		if qosDesc, err := qosFlow.BuildNasQoSDesc(operation); err != nil {
			logger.GsmLog.Warnf("Create QoS Desc from qos flow error: %s\n", err)
		} else {
			authDescs = append(authDescs, qosDesc)
		}
	...
	if len(authDescs) > 0 {
		...
		pDUSessionModificationCommand.AuthorizedQosFlowDescriptions.SetLen(uint16(len(qosDescBytes)))
		pDUSessionModificationCommand.AuthorizedQosFlowDescriptions.SetQoSFlowDescriptions(qosDescBytes)
	}
